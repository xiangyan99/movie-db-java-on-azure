...
az group create -n ${w_us_group} -l ${WEST_US}
create_webapp ${w_us_group} westus
create_kubernetes ${w_us_group} ${ACS_NAME}
...
wait_till_kubernetes_created ${w_us_group} ${ACS_NAME}
[[ $? -ne 0 ]] && return 1
...
if [[ -n "$MS_CORP" ]]; then
  ...
  allow_acs_nsg_access "Internet" "${w_us_group}"
...
create_secrets_in_kubernetes ${w_us_group} ${ACS_NAME}
