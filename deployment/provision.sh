...
create_kubernetes ${e_us_group} ${AKS_NAME}
...
create_kubernetes ${w_eu_group} ${AKS_NAME}

print_banner 'Start provisioning Kubernetes cluster for Jenkins if not exist...'
az group create -n ${jenkins_group} -l ${EAST_US}
create_kubernetes ${jenkins_group} ${AKS_NAME}

print_banner 'Wait until resource provisioning is finished...'
wait_till_kubernetes_created ${e_us_group} ${AKS_NAME}
[[ $? -ne 0 ]] && return 1

wait_till_kubernetes_created ${w_eu_group} ${AKS_NAME}
[[ $? -ne 0 ]] && return 1

wait_till_kubernetes_created ${jenkins_group} ${AKS_NAME}
[[ $? -ne 0 ]] && return 1
...
create_secrets_in_kubernetes ${e_us_group} ${AKS_NAME}
create_secrets_in_kubernetes ${w_eu_group} ${AKS_NAME}

print_banner 'Deploy Jenkins cluster if not exist...'
deploy_jenkins ${jenkins_group} ${AKS_NAME}
